...
def _kernel_io_bytes(batch, seq, heads, kv_lora_rank, qk_rope_head_dim, q_len=1):
...
def _trtllm_mla_io_bytes(batch, seq, heads, kv_lora_rank, qk_rope_head_dim, q_len=1, elem_bytes=2):
    ...
    kv = batch * seq * head_dim * elem_bytes  # ckv + kpe paged caches
...
def run_one_trtllm(batch, seq, heads, q_len=1):
    ...
    pv_dim = kv_lora_rank
    flops = 2 * batch * q_len * heads * seq * (qk_dim + pv_dim)
    tflops = flops / avg_ms / 1e9
    bytes_per_call = _trtllm_mla_io_bytes(
        batch, seq, heads, kv_lora_rank, qk_rope_head_dim, q_len, elem_bytes
    )
    gb_s = bytes_per_call / (avg_ms * 1e-3) / 1e9
    hbm_pct = 100.0 * gb_s / HBM_PEAK_GB_S
    print(
        f"backend={label:>12s} bs={batch:>3d} seq={seq:>5d} heads={heads:>3d} qlen={q_len:>2d}: "
...
def run_one(batch, seq, heads, backend, q_len=1):
    ...
    (
        ...
        seq_lens=[seq] * batch, num_heads=heads, seed=8, query_len_per_seq=q_len
    )
    try:
        ...
        bytes_per_call = _kernel_io_bytes(batch, seq, heads, kv_lora_rank, qk_rope_head_dim, q_len)
        gb_s = bytes_per_call / (avg_ms * 1e-3) / 1e9
        hbm_pct = 100.0 * gb_s / HBM_PEAK_GB_S
        print(
            f"backend={backend:>12s} bs={batch:>3d} seq={seq:>5d} heads={heads:>3d} qlen={q_len:>2d}: "
            f"{avg_ms:>7.3f} ms  {tflops:>6.2f} TFLOP/s  "
            f"HBM {gb_s:>6.1f} GB/s ({hbm_pct:>4.1f}% of {HBM_PEAK_GB_S:.0f})",
            flush=True,
        )
